...
def test_8_chars_with_tilde_case_password_should_raise_exception():
    # given
    validator = PasswordValidator(content="c~abca1Safg2A")
    with pytest.raises(Exception):  # then
        # when
        validator.password_validate()


def test_8_chars_with_circumflex_case_password_should_raise_exception():
    # given
    validator = PasswordValidator(content="ca^ca1SGSj")
    with pytest.raises(Exception):  # then
        # when
        validator.password_validate()


def test_8_chars_with_slash_case_password_should_raise_exception():
    # given
    validator = PasswordValidator(content="ca/ca1S2")
    with pytest.raises(Exception):  # then
        # when
        validator.password_validate()
